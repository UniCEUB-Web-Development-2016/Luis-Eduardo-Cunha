...
class UsuarioController
{
    ...
    public function procurar($requestt)
    {
        $params = $requestt->get_params();
        if ($this->isValid($params)) {
            $usuario = new Usuario(
                $params["email"],
                $params["senha"]);
    ...
    public function search($requestt)
    {
        $params = $requestt->get_params();
        ...
        $db = new DatabaseConnector("localhost", "redeSocial", "mysql", "", "root", "");
        $conn = $db->getConnection();
        $result = $conn->query("SELECT email, senha FROM tb_usuario WHERE ".$crit);
